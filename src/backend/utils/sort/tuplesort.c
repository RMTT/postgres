...
#define PARALLEL_SORT(state)                                                   \
  ((state)->shared == NULL ? 0 : (state)->worker >= 0 ? 1 : 2)
...
#define INITIAL_MEMTUPSIZE                                                     \
  Max(1024, ALLOCSET_SEPARATE_THRESHOLD / sizeof(SortTuple) + 1)
...
struct Tuplesortstate {
   ...
   */
  void (*readtup)(Tuplesortstate *state, SortTuple *stup, int tapenum,
                  unsigned int len);
...
static void readtup_heap(Tuplesortstate *state, SortTuple *stup, int tapenum,
                         unsigned int len);
...
static void writetup_cluster(Tuplesortstate *state, int tapenum,
                             SortTuple *stup);
static void readtup_cluster(Tuplesortstate *state, SortTuple *stup, int tapenum,
                            unsigned int len);
...
static void readtup_index(Tuplesortstate *state, SortTuple *stup, int tapenum,
                          unsigned int len);
static int comparetup_datum(const SortTuple *a, const SortTuple *b,
                            Tuplesortstate *state);
static void copytup_datum(Tuplesortstate *state, SortTuple *stup, void *tup);
static void writetup_datum(Tuplesortstate *state, int tapenum, SortTuple *stup);
static void readtup_datum(Tuplesortstate *state, SortTuple *stup, int tapenum,
                          unsigned int len);
...
#define ST_COMPARE(a, b, ssup)                                                 \
  ApplySortComparator((a)->datum1, (a)->isnull1, (b)->datum1, (b)->isnull1,    \
                      (ssup))
...
static Tuplesortstate *tuplesort_begin_common(int workMem,
   ...
   */
  maincontext = AllocSetContextCreate(CurrentMemoryContext, "TupleSort main",
   ...
   */
  sortcontext = AllocSetContextCreate(maincontext, "TupleSort sort",
                                      ALLOCSET_DEFAULT_SIZES);
...
static void tuplesort_begin_batch(Tuplesortstate *state) {
   ...
   */
  state->tuplecontext = AllocSetContextCreate(
      state->sortcontext, "Caller tuples", ALLOCSET_DEFAULT_SIZES);
  ...
  if (state->memtuples == NULL) {
    state->memtuples =
        (SortTuple *)palloc(state->memtupsize * sizeof(SortTuple));
...
Tuplesortstate *tuplesort_begin_heap(TupleDesc tupDesc, int nkeys,
                                     AttrNumber *attNums, Oid *sortOperators,
                                     Oid *sortCollations, bool *nullsFirstFlags,
                                     int workMem, SortCoordinate coordinate,
                                     bool randomAccess) {
  Tuplesortstate *state =
      tuplesort_begin_common(workMem, coordinate, randomAccess);
  MemoryContext oldcontext;
  int i;
...
#ifdef TRACE_SORT
  if (trace_sort)
    elog(LOG, "begin tuple sort: nkeys = %d, workMem = %d, randomAccess = %c",
         nkeys, workMem, randomAccess ? 't' : 'f');
#endif

  state->nKeys = nkeys;

  TRACE_POSTGRESQL_SORT_START(HEAP_SORT, false, /* no unique check */
                              nkeys, workMem, randomAccess,
                              PARALLEL_SORT(state));
...
Tuplesortstate *tuplesort_begin_cluster(TupleDesc tupDesc, Relation indexRel,
                                        int workMem, SortCoordinate coordinate,
                                        bool randomAccess) {
  Tuplesortstate *state =
      tuplesort_begin_common(workMem, coordinate, randomAccess);
  ...
  if (trace_sort)
    elog(LOG, "begin tuple sort: nkeys = %d, workMem = %d, randomAccess = %c",
         RelationGetNumberOfAttributes(indexRel), workMem,
         randomAccess ? 't' : 'f');
#endif

  state->nKeys = IndexRelationGetNumberOfKeyAttributes(indexRel);

  TRACE_POSTGRESQL_SORT_START(CLUSTER_SORT, false, /* no unique check */
                              state->nKeys, workMem, randomAccess,
                              PARALLEL_SORT(state));
...
Tuplesortstate *tuplesort_begin_index_btree(Relation heapRel, Relation indexRel,
                                            bool enforceUnique, int workMem,
                                            SortCoordinate coordinate,
                                            bool randomAccess) {
  Tuplesortstate *state =
      tuplesort_begin_common(workMem, coordinate, randomAccess);
...
#ifdef TRACE_SORT
  if (trace_sort)
    elog(LOG, "begin index sort: unique = %c, workMem = %d, randomAccess = %c",
         enforceUnique ? 't' : 'f', workMem, randomAccess ? 't' : 'f');
#endif

  state->nKeys = IndexRelationGetNumberOfKeyAttributes(indexRel);

  TRACE_POSTGRESQL_SORT_START(INDEX_SORT, enforceUnique, state->nKeys, workMem,
                              randomAccess, PARALLEL_SORT(state));
...
Tuplesortstate *tuplesort_begin_index_hash(Relation heapRel, Relation indexRel,
...
Tuplesortstate *tuplesort_begin_index_gist(Relation heapRel, Relation indexRel,
                                           int workMem,
                                           SortCoordinate coordinate,
                                           bool randomAccess) {
  Tuplesortstate *state =
...
#ifdef TRACE_SORT
  if (trace_sort)
    elog(LOG, "begin index sort: workMem = %d, randomAccess = %c", workMem,
         randomAccess ? 't' : 'f');
#endif
...
Tuplesortstate *tuplesort_begin_datum(Oid datumType, Oid sortOperator,
                                      Oid sortCollation, bool nullsFirstFlag,
                                      int workMem, SortCoordinate coordinate,
                                      bool randomAccess) {
  Tuplesortstate *state =
      tuplesort_begin_common(workMem, coordinate, randomAccess);
...
#ifdef TRACE_SORT
  if (trace_sort)
    elog(LOG, "begin datum sort: workMem = %d, randomAccess = %c", workMem,
         randomAccess ? 't' : 'f');
#endif

  state->nKeys = 1; /* always a one-column sort */

  TRACE_POSTGRESQL_SORT_START(DATUM_SORT, false, /* no unique check */
 ...
 */
bool tuplesort_used_bound(Tuplesortstate *state) { return state->boundUsed; }
...
static bool grow_memtuples(Tuplesortstate *state) {
   ...
   */
  if (state->availMem <
      (int64)((newmemtupsize - memtupsize) * sizeof(SortTuple)))
    goto noalloc;

  /* OK, do it */
  FREEMEM(state, GetMemoryChunkSpace(state->memtuples));
  state->memtupsize = newmemtupsize;
  state->memtuples = (SortTuple *)repalloc_huge(
      state->memtuples, state->memtupsize * sizeof(SortTuple));
  USEMEM(state, GetMemoryChunkSpace(state->memtuples));
  if (LACKMEM(state))
...
void tuplesort_putindextuplevalues(Tuplesortstate *state, Relation rel,
  ...
  original =
      index_getattr(tuple, 1, RelationGetDescr(state->indexRel), &stup.isnull1);
  ...
  if (!state->sortKeys || !state->sortKeys->abbrev_converter || stup.isnull1) {
    ...
    for (i = 0; i < state->memtupcount; i++) {
      ...
      tuple = mtup->tuple;
      mtup->datum1 = index_getattr(tuple, 1, RelationGetDescr(state->indexRel),
                                   &mtup->isnull1);
    }
  }
...
void tuplesort_putdatum(Tuplesortstate *state, Datum val, bool isNull) {
  ...
  if (isNull || !state->tuples) {
    ...
    if (!state->sortKeys->abbrev_converter) {
      ...
      /* Store abbreviated key representation */
      stup.datum1 =
          state->sortKeys->abbrev_converter(original, state->sortKeys);
    } else {
      /* Abort abbreviation */
      int i;
...
void tuplesort_performsort(Tuplesortstate *state) {
...
#ifdef TRACE_SORT
  if (trace_sort)
    elog(LOG, "performsort of worker %d starting: %s", state->worker,
  ...
  if (trace_sort) {
    if (state->status == TSS_FINALMERGE)
      ...
      elog(LOG, "performsort of worker %d done: %s", state->worker,
           pg_rusage_show(&state->ru_start));
  }
#endif
 ...
 */
static bool tuplesort_gettuple_common(Tuplesortstate *state, bool forward,
                                      SortTuple *stup) {
  unsigned int tuplen;
  size_t nmoved;
  ...
  case TSS_SORTEDONTAPE:
    ...
    if (state->eof_reached) {
       ...
       */
      nmoved = LogicalTapeBackspace(state->tapeset, state->result_tape,
 ...
 */
bool tuplesort_getdatum(Tuplesortstate *state, bool forward, Datum *val,
                        bool *isNull, Datum *abbrev) {
  MemoryContext oldcontext = MemoryContextSwitchTo(state->sortcontext);
  SortTuple stup;
...
static void inittapes(Tuplesortstate *state, bool mergeruns) {
  ...
  state->tapeset = LogicalTapeSetCreate(
      maxTapes, false, NULL, state->shared ? &state->shared->fileset : NULL,
      state->worker);
...
static void mergeruns(Tuplesortstate *state) {
  ...
  state->memtuples = (SortTuple *)MemoryContextAlloc(
      state->maincontext, numInputTapes * sizeof(SortTuple));
  USEMEM(state, GetMemoryChunkSpace(state->memtuples));
  ...
  if (trace_sort)
    elog(LOG,
         "worker %d using " INT64_FORMAT
         " KB of memory for read buffers among %d input tapes",
...
#ifdef TRACE_SORT
  if (trace_sort)
    elog(LOG, "worker %d starting quicksort of run %d: %s", state->worker,
...
#ifdef TRACE_SORT
  if (trace_sort)
    elog(LOG, "worker %d finished quicksort of run %d: %s", state->worker,
         state->currentRun, pg_rusage_show(&state->ru_start));
#endif
...
#ifdef TRACE_SORT
  if (trace_sort)
    elog(LOG, "worker %d finished writing run %d to tape %d: %s", state->worker,
         state->currentRun, state->destTape, pg_rusage_show(&state->ru_start));
#endif
...
void tuplesort_markpos(Tuplesortstate *state) {
  ...
  case TSS_SORTEDINMEM:
    ...
    break;
  case TSS_SORTEDONTAPE:
    LogicalTapeTell(state->tapeset, state->result_tape, &state->markpos_block,
...
void tuplesort_restorepos(Tuplesortstate *state) {
  ...
  case TSS_SORTEDINMEM:
    ...
    break;
  case TSS_SORTEDONTAPE:
    LogicalTapeSeek(state->tapeset, state->result_tape, state->markpos_block,
                    state->markpos_offset);
    state->eof_reached = state->markpos_eof;
    break;
  default:
...
static unsigned int getlen(Tuplesortstate *state, int tapenum, bool eofOK) {
  ...
  if (LogicalTapeRead(state->tapeset, tapenum, &len, sizeof(len)) !=
      sizeof(len))
...
static int comparetup_heap(const SortTuple *a, const SortTuple *b,
  ...
  compare = ApplySortComparator(a->datum1, a->isnull1, b->datum1, b->isnull1,
                                sortKey);
  if (compare != 0)
    return compare;
...
static void copytup_heap(Tuplesortstate *state, SortTuple *stup, void *tup) {
  ...
  original = heap_getattr(&htup, state->sortKeys[0].ssup_attno, state->tupDesc,
                          &stup->isnull1);
  ...
  if (!state->sortKeys->abbrev_converter || stup->isnull1) {
    ...
    for (i = 0; i < state->memtupcount; i++) {
      ...
      htup.t_data =
          (HeapTupleHeader)((char *)mtup->tuple - MINIMAL_TUPLE_OFFSET);

      mtup->datum1 = heap_getattr(&htup, state->sortKeys[0].ssup_attno,
                                  state->tupDesc, &mtup->isnull1);
    }
  }
}
...
static void readtup_heap(Tuplesortstate *state, SortTuple *stup, int tapenum,
  ...
  htup.t_len = tuple->t_len + MINIMAL_TUPLE_OFFSET;
  htup.t_data = (HeapTupleHeader)((char *)tuple - MINIMAL_TUPLE_OFFSET);
  stup->datum1 = heap_getattr(&htup, state->sortKeys[0].ssup_attno,
...
static void copytup_cluster(Tuplesortstate *state, SortTuple *stup, void *tup) {
  ...
  original = heap_getattr(tuple, state->indexInfo->ii_IndexAttrNumbers[0],
                          state->tupDesc, &stup->isnull1);
  ...
  if (!state->sortKeys->abbrev_converter || stup->isnull1) {
    ...
    for (i = 0; i < state->memtupcount; i++) {
      ...
      mtup->datum1 =
          heap_getattr(tuple, state->indexInfo->ii_IndexAttrNumbers[0],
                       state->tupDesc, &mtup->isnull1);
    }
  }
}
...
static void writetup_cluster(Tuplesortstate *state, int tapenum,
  ...
  LogicalTapeWrite(state->tapeset, tapenum, &tuple->t_self,
...
static void readtup_cluster(Tuplesortstate *state, SortTuple *stup, int tapenum,
  ...
  tuple->t_len = t_len;
  LogicalTapeReadExact(state->tapeset, tapenum, &tuple->t_self,
                       sizeof(ItemPointerData));
  ...
  if (state->indexInfo->ii_IndexAttrNumbers[0] != 0)
    stup->datum1 = heap_getattr(tuple, state->indexInfo->ii_IndexAttrNumbers[0],
                                state->tupDesc, &stup->isnull1);
}
...
static int comparetup_index_btree(const SortTuple *a, const SortTuple *b,
  ...
  if (state->enforceUnique && !equal_hasnull) {
    ...
    ereport(ERROR, (errcode(ERRCODE_UNIQUE_VIOLATION),
                    ...
                    errtableconstraint(state->heapRel, RelationGetRelationName(
                                                           state->indexRel))));
  }
...
static void writetup_index(Tuplesortstate *state, int tapenum,
  ...
  LogicalTapeWrite(state->tapeset, tapenum, (void *)&tuplen, sizeof(tuplen));
  LogicalTapeWrite(state->tapeset, tapenum, (void *)tuple,
...
static void readtup_index(Tuplesortstate *state, SortTuple *stup, int tapenum,
  ...
  stup->tuple = (void *)tuple;
  /* set up first-column key value */
  stup->datum1 = index_getattr(tuple, 1, RelationGetDescr(state->indexRel),
                               &stup->isnull1);
}
...
static int comparetup_datum(const SortTuple *a, const SortTuple *b,
  ...
  if (state->sortKeys->abbrev_converter)
    compare = ApplySortAbbrevFullComparator(
        PointerGetDatum(a->tuple), a->isnull1, PointerGetDatum(b->tuple),
        b->isnull1, state->sortKeys);
...
static void writetup_datum(Tuplesortstate *state, int tapenum,
  ...
  if (stup->isnull1) {
    ...
    waddr = stup->tuple;
    tuplen =
        datumGetSize(PointerGetDatum(stup->tuple), false, state->datumTypeLen);
    Assert(tuplen != 0);
  }

  writtenlen = tuplen + sizeof(unsigned int);

  LogicalTapeWrite(state->tapeset, tapenum, (void *)&writtenlen,
                   sizeof(writtenlen));
...
static void leader_takeover_tapes(Tuplesortstate *state) {
  ...
  state->tapeset = LogicalTapeSetCreate(nParticipants + 1, false, shared->tapes,
